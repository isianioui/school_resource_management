...
public class dashboardController {
    ...
    @FXML private TableColumn<Report , String > reportDataColumn;
    ...
    @FXML private TextField reportType;
    @FXML private TextField reportData;
    @FXML
    ...
    private void updateViewVisibility(boolean res, boolean resource, boolean event, boolean stats, boolean users,boolean report) {
    ...
    private void showUsers() {
        ...
        updateViewVisibility(false, false, false, false, true , false);
    ...
    private void setupTableColumns() {
        ...
        reportDataColumn.setCellValueFactory(new PropertyValueFactory<>("data"));
        reportDateColumn.setCellValueFactory(new PropertyValueFactory<>("generatedAt"));
    ...
    private void showReservations() {
        clearAllViews();
        if (!"admin".equals(currentUser.getRole())) {
            reservationActionsColumn.setVisible(false); // Hide the Actions column
        }
        updateViewVisibility(true, false, false, false, false, false);
    ...
    private void showResources() {
        clearAllViews();
        updateViewVisibility(false, true, false, false, false, false);
    ...
    private void showEvents() {
        clearAllViews();
        updateViewVisibility(false, false, true, false, false , false);
    ...
    private void showReports() {
        clearAllViews();
        updateViewVisibility(false, false, false, false, false , true);
        ObservableList<Report> reports = FXCollections.observableArrayList(
                reportDAO.getAllReports());
        reportsTable.setItems(reports); // Corrected the typo
    }
...
@FXML private VBox reportFormView;
@FXML
...
private void handleSaveReport() {
    ...
    if(validateRportInputs()){
    String enteredReportType = reportType.getText();
    String enteredReportData = reportData.getText();
    java.sql.Date selectedDate = java.sql.Date.valueOf(datePicker.getValue());  // Convert LocalDate to SQL Date

    if (enteredReportType != null && !enteredReportType.isEmpty() && selectedDate != null) {
        // Call the saveReport method in the DAO to save the report
        boolean success = reportDAO.saveReport(enteredReportType,enteredReportData, selectedDate);
        ...
        if (success) {
            ...
            // Optionally, you can clear the fields or show a success message here
        } else {
...
private void refreshReportsTable() {
    ...
    for (Report rep : reports) {
        System.out.println("Report ID: " + rep.getReportId() + 
                         " Report Type: " + rep.getReportType()+
                         " Report Data: " + rep.getData()+
                         " Report Date: " + rep.getGeneratedAt());
    }
    ...
    public void setUser(User user) {
        ...
        if (!"admin".equals(currentUser.getRole())) {
            ...
            usersButton.setVisible(false); // Hide the Users button
        }
...
private boolean validateRportInputs() {
    if (reportType.getText() == null) {
        showAlert("Please select a resource");
        return false;
    }
    if (datePicker.getValue() == null) {
...
// Based on your DashboardController code, here's how to fix the reservation form visibility issue. Add this method to handle clearing all views before showing new content:
